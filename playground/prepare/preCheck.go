...
func PreCheck() {
    ...
    if !strings.Contains(res, "mysql") {
        fmt.Println("Detect MySQL client not exists.")
	errFlg = 1
    }
	...
	if utl.PortUsed(portStr) {
	    fmt.Println("Detect the port " + portStr + " used.")
	    errFlg = 1
	}
    }
    ...
    if err == nil {
        fmt.Printf("Detect the folder %s exists, please romove it first using [rm -rf %s]", playgroundFolder, playgroundFolder)
	errFlg = 1
    }
    ...
    if err != nil {
	fmt.Println("mkdir /root/.starrocks-controller")
    ...
    if err != nil {
        fmt.Println("mkdir /root/.starrocks-controller/download")
